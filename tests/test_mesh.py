...
import numpy as np
import pytest
from pybem.mesh import Mesh


@pytest.mark.parametrize(
    "nodes, elements, corners",
    [
        (
            [[0, 0], [1, 0], [1, 1]],
            [[0, 1], [1, 2]],
...
@pytest.mark.parametrize(
    "nodes, elements, center",
    [
        ([[0, 0], [1, 0], [1, 4]], [[0, 1], [1, 2]], np.array([[0.5, 0], [1, 2]])),
...
@pytest.mark.parametrize(
    "nodes, elements, admittances, reference",
    [
...
def test_mesh_admittances(nodes, elements, admittances, reference):
    np.testing.assert_allclose(
        reference, Mesh(nodes, elements, admittances).admittances
    )
