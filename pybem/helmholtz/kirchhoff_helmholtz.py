...
def admitant_2d_integral(mesh, idx, point, k, rho, c):
    n = mesh.normals[idx]
...
def admitant_2d_matrix_element(mesh, row_idx, col_idx, k, rho, c):
...
def kirchhoff_helmholtz_solver(mesh, p_incoming, grad_p_incoming, k, rho, c):
    matrix = complex_system_matrix(admitant_2d_matrix_element, mesh, k, rho, c)
